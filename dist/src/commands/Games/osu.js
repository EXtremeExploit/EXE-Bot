...
export default class {
    constructor(client, msg) {
        ...
        //#region Mode
        let mode = 0;
        if (msg.content.includes(`--std`))
            mode = 0;
        if (msg.content.includes(`--taiko`))
            mode = 1;
        if (msg.content.includes(`--ctb`))
            mode = 2;
        if (msg.content.includes(`--mania`))
            mode = 3;
        //#endregion
        //#region Endpoint
        let endpoint = 'user';
        if (msg.content.includes('--best'))
            endpoint = 'best';
        if (msg.content.includes('--recent'))
            endpoint = 'recent';
        ///endregion
        //#region User
        let user;
                    ...
                    osuApi.getUser({
                        ...
                        if (user.id == undefined) {
                            ...
                            msg.channel.send(new discord.MessageEmbed()
                                ...
                                .addField(`General`, `**Country:** ${iso31661alpha2.getCountry(user.country)}\n` +
                    ...
                    osuApi.getUserBest({
                        ...
                        for (let i = 0; i < 5; i++) {
                            ...
                            desc += `**${i + 1}. ${play.pp}pp**\n[${play.beatmap.artist} - ${play.beatmap.title} [${play.beatmap.version}]](https://osu.ppy.sh/s/${play.beatmap.beatmapSetId})${mods}\n` +
                                ...
                                `${(play.accuracy * 100).toFixed(2)}% / ${play.rank} / ${date}\n\n`;
                        }
                        let modestr;
                        switch (mode) {
                            case 0:
            ...
            case 'recent': {
                osuApi.getUserRecent({
                    ...
                    for (let i = 0; i < 5; i++) {
                        ...
                        desc += `**${i + 1}. ${date}**\n[${play.beatmap.artist} - ${play.beatmap.title} [${play.beatmap.version}]](https://osu.ppy.sh/s/${play.beatmap.beatmapSetId})${mods}\n` +
                            `${play.score} / ${play.maxCombo} (${play.beatmap.maxCombo}) {${play.counts[300]}x300 ${play.counts[100]}x100 ${play.counts[50]}x50 ${play.counts.miss}xMiss}\n` +
                            `${(play.accuracy * 100).toFixed(2)}% / ${play.rank}\n\n`;
                    }
                    let modestr;
                    switch (mode) {
                        case 0:
