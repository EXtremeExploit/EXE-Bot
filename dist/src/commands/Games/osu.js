...
export default class {
    constructor(client, msg) {
        ...
        //#region Mode
        let mode = 0;
        if (msg.content.includes(`--mode std`))
            mode = 0;
        if (msg.content.includes(`--mode taiko`))
            mode = 1;
        if (msg.content.includes(`--mode ctb`))
            mode = 2;
        if (msg.content.includes(`--mode mania`))
            mode = 3;
        //#endregion
        //#region User
        let user;
        ...
        osuApi.getUser({
            ...
            if (user.id == undefined) {
                ...
                msg.channel.send(new discord.MessageEmbed()
                    ...
                    .addField(`Count Ranks`, `**SS/SSH:** ${user.counts.SS}/${user.counts.SSH}\n` +
                    `**S/SH:** ${user.counts.S}/${user.counts.SH}\n` +
                    `**A:** ${user.counts.A}`, true)
                    .addField(`Ranks`, `**Global:** ${user.pp.rank}\n` +
                    `**Country:** ${user.pp.countryRank}`, true)
                    .addField(`Count Notes`, `**300:** ${user.counts[300]}\n` +
                    `**100:** ${user.counts[100]}\n` +
