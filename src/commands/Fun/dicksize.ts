...
export default class {
	constructor(client: discord.Client, msg: discord.Message) {
		...
		let comment: string;
		switch (true) {
			case length == 1: comment = xsmall[random(xsmall.length) - 1]; break;
			case length <= 3: comment = small[random(small.length - 1)]; break;
			case length <= 5: comment = smedium[random(smedium.length - 1)]; break;
			case length <= 7: comment = medium[random(medium.length - 1)]; break;
			case length <= 9: comment = large[random(large.length - 1)]; break;
			case length == 10: comment = xlarge[random(xlarge.length - 1)]; break;
		}
