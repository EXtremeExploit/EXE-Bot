...
export default class {
    GetToken() {
        if (!process.env.token)
            new Error(`Token should be given in an enviromental variable`);
    ...
    GetGoogle() {
        if (!process.env.GoogleCseID)
            new Error(`GoogleCseID should be given in an enviromental variable`);
        if (!process.env.GoogleAppApiKey)
            new Error(`GoogleAppApiKey should be given in an enviromental variable`);
        return {
